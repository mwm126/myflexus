...
private:
  ...
  void createTracers() {
    ...
    for (int32_t ii = 0, simics_cpu_no = 0; ii < theNumCPUs ; ++ii, ++simics_cpu_no) {
      ...
      theTracers[ii]->init(cpu, ii, toL1D, toL1I, toNAW, theWhiteBoxDebug, theWhiteBoxPeriod);
    }
  }
...
SimicsTracerManager * SimicsTracerManager::construct(int32_t aNumCPUs
  ...
  return new SimicsTracerManagerImpl(aNumCPUs, toL1D, toL1I, toDMA, toNAW, aWhiteBoxDebug, aWhiteBoxPeriod);
}
